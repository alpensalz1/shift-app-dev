...
```typescript
type EmploymentType = '社員' | 'アルバイト' | '役員' | 'システム管理者'
```
...
16. **`manage/page.tsx` — dateMap・fixedMap・closedDates・halfStatus の正規化漏れ**
    ...
    - `closedDatesAuto`: `AutoGenerateTab` の closedDates格納も `.substring(0, 10)` に正規化
...
- **雇用形態の網羅性**: `社員 / アルバイト / 役員 / システム管理者` の4種のみ。`長期` は廃止済み（コミット参照）
...
# ② デモ環境（https://shift-app-demo.vercel.app）で動作確認
# ③ 問題なければ本番にpush
...
| リモート | GitHub リポジトリ | Vercel URL | Supabase |
|---------|-----------------|------------|---------|
| `dev`    | shift-app-dev   | shift-app-demo.vercel.app | ubfgnrnfky... |
| `origin` | shift-app       | shift-app-beta.vercel.app | wkpeozzvdx... |
